...
if (! function_exists('sc_custom_image')) {
	function sc_custom_image($attr, $content = null)
	{
		extract(shortcode_atts(array(
		...
		// This variable has been safely escaped above in this function
		$image_output_escapes = '<img class="scale-with-grid sss" src="'. esc_url($src) .'" alt="'. esc_attr($alt) .'" title="'. esc_attr($title) .'" '. $width_escaped .' '. $height_escaped .'/>';
		$image_before_escapes = '<img class="scale-with-grid sss" src="'. esc_url($src) .'" alt="'. esc_attr($alt) .'" title="'. esc_attr($title) .'" '. $width_escaped .' '. $height_escaped .'/>';
		// $background_color_before = $color;
		// output -----
		...
		if ($animate) {
			...
			if ($link || $link_image) {
				...
				// This variable has been safely escaped above in this function
				$output .= '<div class="image_frame image_item scale-with-grid custom-img-widget'. esc_attr($class_div) .'" '. $style_escaped .'>';
					...
					$output .= '<div class="image_wrapper">';
						...
						$output .= '<a href="'. esc_url($link_all) .'" '. $rel_escaped .' '. $target_escaped .'>';
							$output .= '<div class="mask"></div>';
							// This variable has been safely escaped above in this function
							$output .= '<div class="thumbnail-img">';
							$output .= '<div class="second-bg" style="background:url('.esc_url($src).');"><div></div></div>';
							$output .= '<div class="image-wbg" style="background:url('.esc_url($src).');"></div>';
						$output .= '</a>';
				...
				// This variable has been safely escaped above in this function
				$output .= '<div class="image_frame image_item no_link scale-with-grid big-cont'. esc_attr($class_div) .'" '. $style_escaped .'>';

					$output .= '<div class="image_wrapper thumbnail-img">';
						// This variable has been safely escaped above in this function
						// $output .= $image_output_escapes;
						// $output .= $image_before_escapes;
						$output .= '<div class="thumbnail-img">';
	        			$output .= '<div class="second-bg '.$classes.'" style="background:url('.esc_url($src).');"><div></div></div>';
						$output .= '<div class="image-wbg" style="background:url('.esc_url($src).');"></div>';
						
	        		$output .= '</div>';
					$output .= '</div>';
					// $output .= '<style>.big-cont .thumbnail-img .second-bg.'.$colorClass.':before{background:'.$color.';}</style>';
...
if (! function_exists('sc_accordion')) {
	function sc_accordion($attr, $content = null)
	{
		...
		$output = '<div class="accordion ssss">';
...
if (! function_exists('mfn_shortcodes')) {
	function mfn_shortcodes(){
		...
		add_shortcode('custom_image', 'sc_custom_image');
